# tests/test_delete_checklist.py
...
class TestDeleteChecklist:
    ...
    def test_delete_checklist_success(self, api_client):
        issue_id = "EDU-13"
        # 1) Сначала создаём несколько пунктов
        items_to_create = [
            {"text": "Первый пункт", "checked": False},
            {"text": "Второй пункт", "checked": True}
        ]
        for itm in items_to_create:
            resp = api_client.post_checklist_item(issue_id, itm)
            assert resp.status_code in [200, 201]

        # Убедимся, что чек-лист не пуст
        get_before = api_client.get_checklist_items(issue_id)
        assert get_before.status_code == 200
        assert isinstance(get_before.json(), list)
        assert len(get_before.json()) >= 2

        # 2) Удаляем чек-лист целиком
        with allure.step("Отправляем DELETE запрос для удаления чек-листа"):
            delete_resp = api_client.delete_checklist_items(issue_id)

        with allure.step("Проверяем, что код ответа 200 или 204"):
            assert delete_resp.status_code in [200, 204]

        # 3) Делаем GET и проверяем, что чек-лист пуст
        with allure.step("GET после удаления должен вернуть пустой массив"):
            get_after = api_client.get_checklist_items(issue_id)
    ...
    def test_delete_checklist_nonexistent_issue(self, api_client):
        issue_id = "NONEXISTENT-999"
        response = api_client.delete_checklist_items(issue_id)
    ...
    def test_delete_empty_checklist(self, api_client):
        issue_id = "EDU-13"
        # Гарантируем, что чек-лист уже пуст:
        api_client.delete_checklist_items(issue_id)
