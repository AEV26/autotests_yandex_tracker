...
class TestIssueLinking:
    ...
    def test_link_issues_success(self, api_client):
        source_id = "TES-1"
        target_id = "TES-2"
        payload = {
        ...
        with allure.step("Отправляем POST запрос для создания связи"):
            response = api_client.link_issues(source_id, payload)
    ...
    def test_link_issues_validation_error(self, api_client, payload):
        response = api_client.link_issues("TES-1", payload)
        assert response.status_code in [400, 422]
        validate(instance=response.json(), schema=error_schema)

    @allure.title("Связывание с несуществующей задачей")
    def test_link_issues_nonexistent(self, api_client):
    ...
    def test_link_issue_to_itself(self, api_client):
        issue_id = "TES-1"
        payload = {"relationship": "relates", "issue": {"key": issue_id}}
        response = api_client.link_issues(issue_id, payload)
        assert response.status_code in [400, 409, 422]
        validate(instance=response.json(), schema=error_schema)

    @allure.title("Попытка создать дублирующую связь")
    def test_duplicate_link(self, api_client):
        issue_id = "TES-1"
        target_id = "TES-2"
        payload = {"relationship": "relates", "issue": {"key": target_id}}

        first = api_client.link_issues(issue_id, payload)
        assert first.status_code in [201, 422]

        second = api_client.link_issues(issue_id, payload)
