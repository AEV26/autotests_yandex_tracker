...
def get_existing_category(api_client_1) -> str:
...
class TestCreateIssueField:
    ...
    def test_create_field_success(self, api_client_1):
        category_id = get_existing_category(api_client_1)
        ...
        with allure.step("Отправляем POST запрос на создание поля"):
            response = api_client_1.create_issue_field(payload)
    ...
    def test_create_field_unauthorized(self, unauthorized_client, api_client_1):
        category_id = get_existing_category(api_client_1)
        suffix = uuid.uuid4().hex[:6]
        payload = {
            "name": {
    ...
    def test_create_field_invalid_type(self, api_client_1):
        category_id = get_existing_category(api_client_1)
        suffix = uuid.uuid4().hex[:6]
        payload = {
            "name": {
    ...
    def test_create_field_empty_name(self, api_client_1):
        category_id = get_existing_category(api_client_1)
        payload = {
            "name": {
                "ru": "",
    ...
    def test_create_field_conflict(self, api_client_1):
        category_id = get_existing_category(api_client_1)
        suffix = uuid.uuid4().hex[:6]
