...
class TestDeleteIssueLink:
    ...
    def test_delete_issue_link_success(self, api_client_1):
        issue_id = "TES-1"
        target_id = "TES-3"
        payload = {
        ...
        with allure.step("Создаём связь между задачами"):
            link_resp = api_client_1.link_issues(issue_id, payload)
            assert link_resp.status_code in [201, 409]
            link_id = link_resp.json().get("id") if link_resp.status_code == 201 else api_client_1.find_link_id(issue_id, target_id)

        with allure.step("Удаляем связь"):
            del_resp = api_client_1.delete_issue_link(issue_id, link_id)
            assert del_resp.status_code == 204

        with allure.step("Проверяем, что связь удалена"):
            links = api_client_1.get_issue_links(issue_id).json()
    ...
    def test_delete_nonexistent_link(self, api_client_1, issue_id, link_id):
        with allure.step(f"Пытаемся удалить несуществующую связь {link_id} у задачи {issue_id}"):
            response = api_client_1.delete_issue_link(issue_id, link_id)
    ...
    def test_delete_link_twice(self, api_client_1):
        issue_id = "TES-1"
        target_id = "TES-4"
        payload = {
            "relationship": "relates",
        ...
        with allure.step("Удаляем связь повторно"):
            second_del = api_client_1.delete_issue_link(issue_id, link_id)
