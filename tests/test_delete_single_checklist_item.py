# tests/test_delete_single_checklist_item.py
...
class TestDeleteSingleChecklistItem:
    ...
    def test_delete_single_item_success(self, api_client):
        issue_id = "EDU-13"
        # 1) Создаём новый пункт, чтобы получить его ID
        ...
        # 2) Удаляем этот пункт
        with allure.step("Отправляем DELETE запрос для удаления пункта чек-листа"):
            del_resp = api_client.delete_single_checklist_item(issue_id, item_id)

        with allure.step("Проверяем, что код ответа 200 или 204"):
            assert del_resp.status_code in [200, 204]

        # 3) GET → убеждаемся, что удалённого пункта нет в списке
    ...
    @pytest.mark.parametrize("issue_id, item_id", [
        ("EDU-13", "NONEXISTENT_ITEM_ID"),        # пункт не существует, но задача есть
        ("NONEXISTENT-999", "000000000000000000") # задача не существует
    ])
    def test_delete_single_item_nonexistent(self, api_client, issue_id, item_id):
        response = api_client.delete_single_checklist_item(issue_id, item_id)
    ...
    def test_delete_single_item_idempotency(self, api_client):
        issue_id = "EDU-13"
        # 1) Создаём новый пункт
        new_item = {"text": "Пункт для идемпотентности", "checked": False}
        create_resp = api_client.post_checklist_item(issue_id, new_item)
        assert create_resp.status_code in [200, 201]
        item_id = create_resp.json()["checklistItems"][-1]["id"]

        # 2) Первый раз удаляем
        first_del = api_client.delete_single_checklist_item(issue_id, item_id)
        assert first_del.status_code in [200, 204]

        # 3) Второй раз удаляем тот же item_id
        second_del = api_client.delete_single_checklist_item(issue_id, item_id)
        # API может вернуть 404 или 204/200 — проверяем, что не падает
        assert second_del.status_code in [200, 204, 404]

    @allure.title("Пограничный случай: удаление с дополнительными параметрами")
    def test_delete_single_item_with_extra_params(self, api_client):
        issue_id = "EDU-13"
        # Создаём пункт, чтобы удалить
        new_item = {"text": "Пункт с параметрами", "checked": False}
        create_resp = api_client.post_checklist_item(issue_id, new_item)
        assert create_resp.status_code in [200, 201]
        item_id = create_resp.json()["checklistItems"][-1]["id"]

        # Передаём лишний параметр в DELETE
        with allure.step("DELETE с ненужным параметром в URL"):
            del_resp = api_client.delete_single_checklist_item(issue_id, item_id, params={"extra": "value"})

        with allure.step("Код ответа должен быть 200 или 204"):
            assert del_resp.status_code in [200, 204]

        # Проверяем, что пункт действительно удалён
        get_resp = api_client.get_checklist_items(issue_id)
