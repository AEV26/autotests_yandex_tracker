...
class TestEditChecklistItem:
    ...
    def test_patch_checklist_item_success(self, api_client_1):
        issue_id = "EDU-13"
        ...
        with allure.step("Отправляем PATCH запрос для обновления пункта чек-листа"):
            patch_resp = api_client_1.patch_checklist_item(issue_id, item_id, updated_payload)
    ...
    @pytest.mark.parametrize("invalid_data", [
        {"text": "", "checked": False},
        {"text": "Текст", "checked": "не_булево"},
        {"unsupportedField": "value"},
    ])
    def test_patch_checklist_item_validation_error(self, api_client_1, invalid_data):
        issue_id = "EDU-13"

        create_resp = api_client_1.post_checklist_item(issue_id, {"text": "Для валидации", "checked": False})
        assert create_resp.status_code in [200, 201]
        created_items = create_resp.json()["checklistItems"]
        item_id = [it for it in created_items if it["text"] == "Для валидации"][-1]["id"]

        response = api_client_1.patch_checklist_item(issue_id, item_id, invalid_data)
    ...
    def test_patch_checklist_item_nonexistent(self, api_client_1):
        issue_id = "EDU-13"
        fake_item_id = "NONEXISTENT_ITEM_ID"
        payload = {"text": "Текст", "checked": False}
        response = api_client_1.patch_checklist_item(issue_id, fake_item_id, payload)
    ...
    def test_patch_checklist_item_partial_update_text(self, api_client_1):
        issue_id = "EDU-13"
        create_resp = api_client_1.post_checklist_item(issue_id, {"text": "Частичный", "checked": False})
        assert create_resp.status_code in [200, 201]
        item = create_resp.json()["checklistItems"][-1]
        item_id = item["id"]

        with allure.step("PATCH: обновляем только текст"):
            patch_resp = api_client_1.patch_checklist_item(issue_id, item_id, {"text": "Только текст"})
        ...
        with allure.step("Код 200 и проверка поля text"):
            ...
            assert "checked" in patched

    @allure.title("Частичное обновление (только checked)")
    def test_patch_checklist_item_partial_update_checked(self, api_client_1):
        issue_id = "EDU-13"
        create_resp = api_client_1.post_checklist_item(issue_id, {"text": "Частичный2", "checked": False})
        assert create_resp.status_code in [200, 201]
        item = create_resp.json()["checklistItems"][-1]
        item_id = item["id"]

        with allure.step("PATCH: обновляем только checked"):
            patch_resp = api_client_1.patch_checklist_item(issue_id, item_id, {"checked": True})
