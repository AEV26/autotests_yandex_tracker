...
class TestAddChecklistItem:
    ...
    def test_create_checklist_item_success(self, api_client_1):
        ...
        with allure.step("Отправляем POST запрос на создание пункта чек-листа"):
            response = api_client_1.post_checklist_item(issue_id, new_item)
    ...
    @pytest.mark.parametrize("invalid_data", [
        {"text": "", "checked": False},
        {"text": "Текст", "checked": "нет"},
        {},
    ])
    def test_create_checklist_item_validation_error(self, api_client_1, invalid_data):
        issue_id = "EDU-13"
        response = api_client_1.post_checklist_item(issue_id, invalid_data)
    ...
    def test_create_checklist_item_nonexistent_issue(self, api_client_1):
        issue_id = "NONEXISTENT-999"
        item = {"text": "Тест", "checked": False}
        response = api_client_1.post_checklist_item(issue_id, item)
