...
class TestChecklistItems:
    ...
    def test_get_checklist_success(self, api_client_1):
        with allure.step("Запрашиваем чек-лист для существующей задачи"):
            response = api_client_1.get_checklist_items("EDU-13")
    ...
    def test_nonexistent_issue(self, api_client_1):
        with allure.step("Запрос чек-листа для несуществующей задачи"):
            response = api_client_1.get_checklist_items("NONEXISTENT-999")
    ...
    def test_pagination(self, api_client_1, per_page):
        with allure.step(f"Запрос с пагинацией (perPage={per_page})"):
            response = api_client_1.get_checklist_items(
                "EDU-13",
